...
const ServicePage = () => {
  const service = webServicesData.find(s => s.id === 'responsive-design');
  const meta = seoData['responsive-design'] || {}; 
  const serviceBasePath = WEB_SERVICES_BASE;
  const serviceListPath = serviceBasePath;
  const servicePath = `${serviceBasePath}/responsive-design`;
  ...
  const relatedServices = useMemo(() => {
    return webServicesData
      .filter(s => s.category === service?.category && s.id !== 'responsive-design')
      .slice(0, 4);
  }, [webServicesData, service]);
