...
const ServicePage = () => {
  const service = industriesData.find(s => s.id === 'beauty-personal-care');
  const meta = seoData['static-website'] || {}; 
  const serviceBasePath = WEB_SERVICES_BASE;
  const serviceListPath = serviceBasePath;
  const servicePath = `${serviceBasePath}/beauty-personal-care`;
  ...
  const relatedServices = useMemo(() => {
    return industriesData
      .filter(s => s.category === service?.category && s.id !== 'beauty-personal-care')
      .slice(0, 4);
