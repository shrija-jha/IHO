...
const themePalette = {
  cyan: { 
    ...
    hex: "#06b6d4",
    titleGradient: "from-[#22d3ee] via-[#06b6d4] to-[#0891b2]"
  }
};
...
const TiltHeading = ({ text, theme }) => {
  ...
  return (
    ...
    >
      <motion.h1 
        style={{ transform: "translateZ(30px)", dropShadow: "0px 10px 30px rgba(6,182,212,0.5)" }}
      ...
      <div className={`absolute inset-0 bg-cyan-500/30 blur-[100px] opacity-80 -z-10 animate-pulse`} />
      <div className={`absolute inset-0 bg-sky-500/20 blur-[150px] opacity-50 -z-20`} />
    </motion.div>
  );
};
    ...
    <div className="space-y-6 text-slate-300 relative z-10 w-full">
      {processedBlocks.map((item, index) => {
        ...
        if (item.type === 'section_heading') {
          return <h3 key={index} className={`text-2xl md:text-3xl font-extrabold ${theme.text} mt-12 mb-4 drop-shadow-[0_0_10px_rgba(6,182,212,0.4)]`}>{item.content}</h3>;
        }
        if (item.type === 'feature_card') {
          return (
            <div key={index} className="my-6 p-6 bg-white/5 border border-white/10 rounded-xl shadow-[0_0_20px_rgba(6,182,212,0.1)] backdrop-blur-md">
...
const TechnologyPage = () => {
  ...
  const pageCategory = "TECHNOLOGY SOLUTIONS";
...
Contact IHO Digital today and take your technology business to the next level.`;
  ...
  return (
    <div className="min-h-screen bg-[#050510] text-slate-200 overflow-x-hidden relative font-sans selection:bg-cyan-500/30">
      <SEO title={`${pageTitle} - IHO Digital`} description={pageDescription} canonical="/industries/technology" />
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none z-0">
        ...
        <motion.div 
          ...
          style={{ background: `radial-gradient(circle, #06b6d4 0%, transparent 70%)`, mixBlendMode: 'screen' }} 
        />
        <motion.div 
          ...
          style={{ background: `radial-gradient(circle, #0891b2 0%, transparent 70%)`, mixBlendMode: 'screen' }} 
        />
      </div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <div className="relative" ref={dropdownRef}>
            ...
            <motion.div 
              ...
              className={`absolute right-0 mt-3 w-56 rounded-xl bg-[#0b0c16] border border-cyan-500/20 shadow-[0_15px_40px_-10px_rgba(6,182,212,0.5)] overflow-hidden z-50 ${isDropdownOpen ? 'pointer-events-auto' : 'pointer-events-none'}`}
            >
              <div className="py-2 flex flex-col bg-white/5 backdrop-blur-xl">
                <Link to="/industries/healthcare" onClick={() => setIsDropdownOpen(false)} className="px-5 py-3 text-sm font-medium text-slate-300 hover:bg-cyan-500/10 hover:text-cyan-400 transition-colors border-l-2 border-transparent hover:border-cyan-500">Healthcare</Link>
                <Link to="/industries/automotive" onClick={() => setIsDropdownOpen(false)} className="px-5 py-3 text-sm font-medium text-slate-300 hover:bg-cyan-500/10 hover:text-cyan-400 transition-colors border-l-2 border-transparent hover:border-cyan-500">Automotive</Link>
                <Link to="/industries/real-estate" onClick={() => setIsDropdownOpen(false)} className="px-5 py-3 text-sm font-medium text-slate-300 hover:bg-cyan-500/10 hover:text-cyan-400 transition-colors border-l-2 border-transparent hover:border-cyan-500">Real Estate</Link>
                <Link to="/industries/ecommerce" onClick={() => setIsDropdownOpen(false)} className="px-5 py-3 text-sm font-medium text-slate-300 hover:bg-cyan-500/10 hover:text-cyan-400 transition-colors border-l-2 border-transparent hover:border-cyan-500">E-Commerce</Link>
                <Link to="/industries/education" onClick={() => setIsDropdownOpen(false)} className="px-5 py-3 text-sm font-medium text-slate-300 hover:bg-cyan-500/10 hover:text-cyan-400 transition-colors border-l-2 border-transparent hover:border-cyan-500">Education</Link>
              </div>
            </motion.div>
          </div>
      ...
      <div className="relative z-10 flex flex-col items-center justify-center text-center px-6 pt-6 pb-20">
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
          <div className="inline-flex items-center gap-3 px-6 py-2 rounded-full border border-white/20 bg-white/5 backdrop-blur-md shadow-[0_0_20px_rgba(255,255,255,0.05)] mb-8">
            <Sparkles className="w-4 h-4 text-cyan-400 drop-shadow-[0_0_8px_#22d3ee]" />
      ...
      <div className="relative z-20 px-6 py-20 max-w-6xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12">
          ...
          <div className="bg-[#0b0c16] border border-white/5 rounded-[2rem] p-10 lg:p-14 flex flex-col items-center justify-center text-center shadow-[0_0_50px_-15px_rgba(6,182,212,0.5)] relative overflow-hidden group">
            <div className="absolute inset-0 bg-gradient-to-br from-cyan-500/10 to-transparent opacity-80" />
            
            <div className="w-20 h-20 rounded-3xl bg-[#16182d] border border-cyan-500/20 flex items-center justify-center shadow-[0_0_30px_rgba(6,182,212,0.4)] mb-8 relative z-10 transition-transform duration-300 group-hover:scale-110">
              <Rocket className="w-8 h-8 text-cyan-400 drop-shadow-[0_0_8px_#22d3ee]" />
            </div>
            ...
            <p className="text-slate-400 leading-relaxed max-w-sm relative z-10">
              Specialized digital strategies designed to maximize growth in the Technology sector.
            </p>
          </div>

          {/* Card 2: Key Solutions */}
          <div className="bg-[#0b0c16] border border-white/5 rounded-[2rem] p-10 flex flex-col shadow-[0_0_50px_-15px_rgba(6,182,212,0.5)] relative overflow-hidden group">
            <div className="absolute inset-0 bg-gradient-to-bl from-cyan-500/10 to-transparent opacity-80" />
            
            <div className="flex items-center gap-5 mb-8 relative z-10">
              <div className="w-14 h-14 rounded-2xl bg-[#16182d] border border-cyan-500/20 flex items-center justify-center shadow-[0_0_20px_rgba(6,182,212,0.4)]">
                <Star className="w-6 h-6 text-cyan-400 drop-shadow-[0_0_8px_#22d3ee]" />
              </div>
            ...
            <div className="space-y-6 mb-10 flex-1 relative z-10">
              {pageFeatures.map((feature, i) => (
                <div key={i} className="flex items-center gap-4">
                  <div className="w-6 h-6 rounded-full bg-cyan-500/10 flex items-center justify-center border border-cyan-500/30 shadow-[0_0_10px_rgba(6,182,212,0.2)]">
            ...
            <button 
              ...
              className="w-full py-4 rounded-xl bg-[#06b6d4] hover:bg-[#0891b2] text-white font-bold text-lg transition-colors shadow-[0_10px_30px_-10px_rgba(6,182,212,0.6)] relative z-10"
            >
              Get Custom Quote
            </button>
          </div>
